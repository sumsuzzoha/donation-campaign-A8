    ...
    return (
        <div>
            <div style={styles.contentColor} className="card card-side bg-base-100 max-h-48 shadow-xl">
                <figure><img className="h-32 ps-2 rounded-lg md:max-h-48 max-w-max" src={image} alt="Movie" /></figure>
                <div className="card-body pt-4">
                    <div className="card-actions justify-start">
                        <button style={styles.contentColor} className="btn btn-xs normal-case">{category}</button>
                    </div>
                    <h2 style={styles.titleColor} className="text-lg md:text-xl font-bold">{title}</h2>
                    <p className="flex items-center font-bold"><BiDollar></BiDollar>{price}.00</p>
                    <div className="card-actions justify-start">
                        <Link to={`/cardDetail/${id}`}>
                            <button style={styles.btnColor} className="btn btn-xs ms:btn-sm">View Details</button>
                        </Link>
